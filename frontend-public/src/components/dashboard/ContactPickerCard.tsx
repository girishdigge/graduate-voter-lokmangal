...
import { ReferredContactsList } from './ReferredContactsList';
import {
  ...
  const handleSubmitReferences = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const addedCount = formattedReferences.length;
        const message =
          addedCount === 1
            ? 'Reference added successfully! They will receive a WhatsApp notification soon.'
            : `${addedCount} references added successfully! They will receive WhatsApp notifications soon.`;
        alert(message);
      }
    } catch (error: any) {
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">Add contact references</p>
          </div>
        </div>
        <div className="flex items-center justify-center py-8">
          <LoadingSpinner size="md" text="Loading your references..." />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="flex items-center">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              Add references and view referred contacts
            </p>
          </div>
        </div>
      ...
      <div>
              ...
              <div className="text-sm text-gray-500">
                {
                  references.filter(
                    (r: Reference) =>
                      r.status === 'CONTACTED' || r.status === 'APPLIED'
                  ).length
                }{' '}
                active
              </div>
                      ...
                      {capabilities.supportsContactPicker ? (
                        <div className="text-center">
                          ...
                          <p className="text-sm text-gray-600 mt-2">
                            Access your device contacts to quickly add
                            references
                          </p>
                        </div>
                      ) : (
        ...
        {/* Referred by Me Section */}
        <div className="mt-8">
          <ReferredContactsList userId={userId} />
        </div>
      </div>
    </div>
  );
